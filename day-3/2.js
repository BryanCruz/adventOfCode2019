...
const [instructionsA, instructionsB] = [
  decodeInstructions(inputA),
  decodeInstructions(inputB)
];

const getLines = instructions =>
  instructions.reduce(
    (prev, curr) => {
      const [array, [x, y]] = prev;
      const [dir, qtt] = curr;
      ...
      return [array.concat([nextLine]), nextPoint];
    },

    [[], [0, 0]]
  );

const visitedPoints = lines =>
  lines.reduce(
    ([visited, qttSteps, [x0, y0]], [[xA, yA], [xB, yB]]) => {
      const [xf, yf] = x0 === xA && y0 === yA ? [xB, yB] : [xA, yA];

      const step = x0 < xf || y0 < yf ? 1 : -1;

      for (let i = x0; i !== xf; i += step) {
        visited.push([[i, yf], qttSteps]);
        qttSteps += 1;
      }

      for (let i = y0; i !== yf; i += step) {
        visited.push([[xf, i], qttSteps]);
        qttSteps += 1;
      }

      return [visited, qttSteps, [xf, yf]];
    },
    [[], 0, [0, 0]]
  );

const [linesA] = getLines(instructionsA);
const [linesB] = getLines(instructionsB);

const [visitedA] = visitedPoints(linesA);
const [visitedB] = visitedPoints(linesB);
...
const intersectionPoint = (
  ...
  if (xV >= x1H && xV <= x2H && yH >= y1V && yH <= y2V) {
    return p;
  }
...
const intersections = linesA
  .flatMap(lineA =>
    linesB.map(lineB => {
      const [[x1A, y1A], [x2A, y2A]] = lineA;
      const [[x1B, y1B], [x2B, y2B]] = lineB;

      if (x1A === x2A && y1B === y2B) {
        return intersectionPoint(lineB, lineA);
      } else if (x1B === x2B && y1A === y2A) {
        return intersectionPoint(lineA, lineB);
      }

      return undefined;
    })
  )
  .filter(v => v !== undefined && v[0] !== 0 && v[1] !== 0)
  .map(
    ([x, y]) =>
      visitedA.find(([[xv, yv]]) => x === xv && y === yv)[1] +
      visitedB.find(([[xv, yv]]) => x === xv && y === yv)[1]
  );
